...
It accepts the command, tokenizes the command line and executes it by creating the child process.
Also implement the additional command ‘typeline’ as 
myshell$ typeline -n filename: It will display the last n lines of the file.
*/
#include <stdio.h>
//#include <sys/wait.h>
#include <unistd.h>
...
int typeline(char *option, char *fname) {
    ...
    for (int i = 0; i < n; i++) {
        lines[i] = (char *)malloc(100); // Adjust the line length as needed
        if (lines[i] == NULL) {
            perror("malloc");
            for (int j = 0; j < i; j++) {
                free(lines[j]);
